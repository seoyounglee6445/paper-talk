============================================================
FILE 1: src/worker.js
Use this as the Cloudflare Worker main file.
============================================================
/*
...
============================================================
FILE 2: public/visium-gpt.html
Use this as the frontend file to fix the first-message disappearing bug.
============================================================
<!doctype html>
<html lang="en">
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .cancel-btn:hover,
    .login-btn:hover,
    .password-btn:hover { transform: translateY(-1px); }
    ...
    .send-btn:hover,
    .login-btn:hover,
    .password-btn:hover { background: var(--primary-dark); }
    ...
    .thread-btn.active { background: #e7edff; }
    ...
    textarea:focus {
      ...
      box-shadow: 0 0 0 3px rgba(17, 25, 184, 0.08);
    }
...
<body>
  <main class="page">
    <section class="top-copy">
      <p id="landingCopy">It answers with Research Paper knowledge added by Paper_Talk.<br>The research database is updated daily with newly indexed publications.</p>
      <p id="usageCopy">Guest usage: Try 3 questions.<br>Signed-in usage: 50 questions per user per month.</p>
    </section>

    <section class="app-shell">
      <aside class="sidebar">
        <button id="newChatBtn" class="new-chat-btn" type="button">+ New Chat</button>
        <div id="threadList" class="thread-list" aria-label="Chat threads"></div>
        <div class="sidebar-note" id="loginNote"></div>
      </aside>

      <section class="chat-card">
        <div class="gpt-header">
          <div>
            <h1 class="gpt-title" id="gptTitle">Paper_Talk Vision GPT</h1>
            <p class="gpt-subtitle" id="gptSubtitle">Ask about papers, validation, gaps, or research ideas.</p>
          </div>
          <div class="usage-pill" id="quotaText">Questions left: --</div>
        </div>

        <div id="passwordPanel" class="password-panel">
          <div>Neuroscience GPT is private. Please enter the password first.</div>
          <div class="password-row">
            <input id="neuroPasswordInput" type="password" placeholder="Password" />
            <button id="neuroPasswordBtn" class="password-btn" type="button">Unlock</button>
          </div>
        </div>

        <div id="messages" class="messages">
          <p class="empty-state" id="emptyState">Start a new conversation with Paper_Talk Vision GPT.</p>
        </div>

        <form id="chatForm" class="chat-form" autocomplete="off">
          <div class="input-wrap">
            <textarea id="messageInput" placeholder="Thinking may take 1-30 sec...&#10;Ask about papers, validation, gaps, or research ideas..."></textarea>
          </div>
  ...
  <script>
    const GPT_PROFILES = {
      paper_talk: {
        title: 'Paper_Talk Vision GPT',
        subtitle: 'Ask about papers, validation, gaps, or research ideas.',
        path: '/visium-gpt.html?gpt=paper_talk'
      },
      neuroscience: {
        title: 'Neuroscience GPT',
        subtitle: 'Specialist GPT using curated neuroscience knowledge.',
        path: '/neuroscience-gpt'
      },
      mitochondria: {
        title: 'Mitochondria GPT',
        subtitle: 'Specialist GPT using curated mitochondria knowledge.',
        path: '/mitochondria-gpt'
      },
      single_cell: {
        title: 'Single-cell GPT',
        subtitle: 'Specialist GPT using curated single-cell knowledge.',
        path: '/single-cell-gpt'
      },
      spatial_biology: {
        title: 'Spatial Biology GPT',
        subtitle: 'Specialist GPT using curated spatial biology knowledge.',
        path: '/spatial-biology-gpt'
      },
      cancer_genomics: {
        title: 'Cancer Genomics GPT',
        subtitle: 'Specialist GPT using curated cancer genomics knowledge.',
        path: '/cancer-genomics-gpt'
      }
    };
    ...
    const gptTitleEl = document.getElementById('gptTitle');
    const gptSubtitleEl = document.getElementById('gptSubtitle');
    ...
    function normalizeGptKey(value) {
      const raw = String(value || 'paper_talk')
        ...
        .replace(/\s+/g, '_')
        .replace(/-/g, '_');
      const key = GPT_ALIASES[raw] || raw;
      return GPT_PROFILES[key] ? key : 'paper_talk';
    }

    const params = new URLSearchParams(location.search);
    const currentGptKey = normalizeGptKey(params.get('gpt') || params.get('gptKey') || params.get('domain') || 'paper_talk');
    const currentProfile = GPT_PROFILES[currentGptKey] || GPT_PROFILES.paper_talk;
    ...
    function showEmptyState(text) {
      ...
      p.id = 'emptyState';
      p.textContent = text || `Start a new conversation with ${currentProfile.title}.`;
    ...
    function appendChatMessage(role, text, options = {}) {
      ...
      div.className = role === 'user' ? 'message user' : role === 'system' ? 'message system' : 'message assistant';
      if (options.pending) div.className += ' pending';
      div.innerHTML = escapeHtml(text);
    ...
    function setBubbleText(bubble, role, text, options = {}) {
      if (!bubble) return;
      bubble.className = role === 'system' ? 'message system' : 'message assistant';
      if (options.pending) bubble.className += ' pending';
    ...
    function updateQuotaLabel(quota) {
      if (!quota) {
        quotaTextEl.textContent = 'Questions left: --';
        return;
      }
      const remaining = Number(quota.remaining ?? Math.max(Number(quota.limit || 0) - Number(quota.used || 0), 0));
      const limit = Number(quota.limit || 0);
      quotaTextEl.textContent = `Questions left: ${remaining}/${limit}`;
    }

    function apiUrl(path, extra = {}) {
      const url = new URL(path, location.origin);
      url.searchParams.set('gptKey', currentGptKey);
    ...
    async function loadMe() {
      try {
        ...
        isGuest = Boolean(data.guest || !data.user);
        updateQuotaLabel(data.quota);

        if (isGuest) {
          loginNoteEl.innerHTML = '<a class="login-btn" style="display:inline-flex;align-items:center;justify-content:center;height:38px;padding:0 14px;color:white;" href="/auth/google">Sign in with Google</a>';
        } else {
          loginNoteEl.textContent = data.user?.email ? `Signed in as ${data.user.email}` : 'Signed in';
        }
      } catch (_) {}
    }
    ...
    async function loadThreads(options = {}) {
      ...
      const skipAutoOpen = Boolean(options.skipAutoOpen);

      if (isGuest) {
        threadListEl.innerHTML = '<div class="thread-row"><button class="thread-btn active" type="button">New chat</button></div>';
        return;
      }

      try {
        const res = await fetch(apiUrl('/api/gpt/threads'), { cache: 'no-store' });
        const data = await readJsonResponse(res);

        if (!data.ok) {
          threadListEl.innerHTML = '<div class="sidebar-note">No saved threads.</div>';
          return;
        }
        ...
        for (const thread of threads) {
          ...
          btn.type = 'button';
          btn.className = 'thread-btn' + (thread.id === currentThreadId ? ' active' : '');
        ...
        // 중요: send 직후에는 현재 messages 영역을 다시 그리지 않습니다.
        // 첫 메시지가 사라지는 원인이 여기서 자동 open/loadMessages 되는 것이었습니다.
        if (skipAutoOpen || preserveCurrentMessages) return;
      } catch (_) {
        threadListEl.innerHTML = '<div class="sidebar-note">Could not load threads.</div>';
      }
    }
    ...
    async function openThread(threadId) {
      ...
      if (!currentThreadId) {
        showEmptyState(`Start a new conversation with ${currentProfile.title}.`);
        await loadThreads({ preserveCurrentMessages: true, skipAutoOpen: true });
        return;
      }

      try {
        const res = await fetch(apiUrl('/api/gpt/messages', { threadId: currentThreadId }), { cache: 'no-store' });
        const data = await readJsonResponse(res);
        if (!data.ok) {
          showEmptyState(data.error || 'Could not load messages.');
          return;
        }
        ...
        showEmptyState(err?.message || String(err));
      }
    }

    async function deleteThread(threadId) {
      if (!threadId || isSending) return;
      const ok = confirm('Delete this chat thread?');
      if (!ok) return;

      try {
        const res = await fetch(apiUrl('/api/gpt/threads/' + encodeURIComponent(threadId)), {
          method: 'DELETE'
        });
        const data = await readJsonResponse(res);
        if (!data.ok) {
    ...
    async function sendMessage(event) {
      ...
      // 중요: input은 비워도 되지만 messages 영역은 절대 비우면 안 됩니다.
      inputEl.value = '';

      // 중요: 첫 메시지도 서버 응답 전 local bubble로 먼저 보여줍니다.
      appendChatMessage('user', message);
      const pendingBubble = appendChatMessage('assistant', 'Thinking may take 1-30 sec...', { pending: true });

      activeCancelId = makeCancelId();
      ...
      try {
        const res = await fetch('/api/gpt/chat', {
          ...
          body: JSON.stringify({
            ...
            gptKey: currentGptKey,
            cancelId: activeCancelId
          })
        });

        const data = await readJsonResponse(res);

        if (data.threadId && data.threadId !== 'guest') {
          currentThreadId = data.threadId;
        }

        if (data.quota) updateQuotaLabel(data.quota);

        if (data.ok) {
          setBubbleText(pendingBubble, 'assistant', data.answer || 'No answer returned.');
        } else if (data.canceled) {
          setBubbleText(pendingBubble, 'system', data.error || 'Canceled by user.');
        } else if (data.private) {
          passwordPanelEl.style.display = 'block';
          setBubbleText(pendingBubble, 'system', data.error || 'Please enter the password first.');
        } else {
          setBubbleText(pendingBubble, 'system', data.error || 'Request failed.');
        }

        // 중요: thread list만 갱신합니다. 현재 messages를 다시 load/render하지 않습니다.
        await loadThreads({ preserveCurrentMessages: true, skipAutoOpen: true });
      } catch (err) {
        if (err && err.name === 'AbortError') {
          setBubbleText(pendingBubble, 'system', 'Canceled by user.');
        } else {
          setBubbleText(pendingBubble, 'system', err?.message || String(err));
        }
      } finally {
    ...
    async function cancelActiveRequest() {
      if (!isSending || !activeCancelId) return;
      const cancelId = activeCancelId;

      try {
        await fetch('/api/gpt/cancel', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            cancelId,
    ...
    async function unlockNeuroscienceGpt() {
      const password = String(neuroPasswordInput.value || '').trim();
      if (!password) return;
      ...
      try {
        ...
        if (data.ok && data.authenticated) {
          ...
          neuroPasswordInput.value = '';
          appendChatMessage('system', 'Neuroscience GPT unlocked. You can ask now.');
        } else {
          appendChatMessage('system', data.error || 'Incorrect password.');
        }
      } catch (err) {
        appendChatMessage('system', err?.message || String(err));
      }
    }
    ...
    function startNewChat() {
      ...
      currentThreadId = '';
      showEmptyState(`Start a new conversation with ${currentProfile.title}.`);
      loadThreads({ preserveCurrentMessages: true, skipAutoOpen: true });
      inputEl.focus();
    }

    function initStaticText() {
      document.title = currentProfile.title;
      gptTitleEl.textContent = currentProfile.title;
      gptSubtitleEl.textContent = currentProfile.subtitle;
      showEmptyState(`Start a new conversation with ${currentProfile.title}.`);

      if (currentGptKey === 'neuroscience') {
        passwordPanelEl.style.display = 'block';
      }
    }
    ...
    neuroPasswordBtn.addEventListener('click', unlockNeuroscienceGpt);
    neuroPasswordInput.addEventListener('keydown', event => {
      if (event.key === 'Enter') unlockNeuroscienceGpt();
    });
