...
1. Start with "Paper_Talk DB evidence used" and list the retrieved source titles.
2. For each major claim, attach the DB source number/title that supports it.
3. Separate DB-supported findings from hypotheses generated by connecting DB sources.
4. Identify knowledge gaps strictly inside the retrieved DB context.
5. Generate only testable hypotheses that follow from the retrieved DB evidence.
6. For each hypothesis, include:
   - hypothesis statement
   - DB evidence link: Source numbers/titles
   - knowledge gap addressed
   - validation plan
   - Novelty score 1-5
   - Feasibility score 1-5
   - Risk score 1-5
7. Do not invent external papers, sample sizes, datasets, mechanisms, or biomarkers not present in the DB context.
8. If evidence is weak, explicitly say "DB evidence is weak because..."
